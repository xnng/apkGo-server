<template>
  <div v-loading="loading">
    <div class="section1">
      ...
      <div class="info">{{app.versionInfo.versionName}} (Build {{app.versionInfo.versionCode}})</div>
      <div class="info">更新于：{{renderTime(app.versionInfo.updateAt)}}</div>
      <el-button type="primary" round class="download" :loading="downLoading" @click="download">下载安装</el-button>
    </div>
    ...
    <div class="section2">
      <hr class="hr" />
      <div class="journal">更新日志</div>
      <div class="journal-text">{{ app.versionInfo.updateText ? app.versionInfo.updateText : '无'}}</div>
      <hr class="hr" />
      <footer>陕ICP备18006529号</footer>
    </div>
  </div>
</template>

<script>
import { getReleaseInfo, downloadapp } from '@/api/app'
...
export default {
  ...
  async mounted () {
    this.loading = true
    try {
      const result = await getReleaseInfo(this.$route.params.id)
  ...
  methods: {
    async download () {
      ...
      try {
        const result = await downloadapp(this.$route.params.id)
        if (result.data.code === 0) {
